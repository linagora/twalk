...
describe('remembering the domain', () => {
	...
	// new tab, or from a bookmark the next day. With the domain in
	// `sessionStorage` that page had no domain, resolved an empty homeserver
	// URL, and failed with a raw `Failed to fetch` (#112). So which store this
	// uses is part of the behaviour, and is asserted rather than assumed.
	function withFakeWindow(run: (stores: { local: Storage; session: Storage }) => void): void {
		const make = (): Storage => {
	...
	it('survives a new tab, which is what a fresh sessionStorage is', () => {
		withFakeWindow((stores) => {
			...
			expect(stores.session.length, 'nothing went where a new tab cannot see it').toBe(0);
