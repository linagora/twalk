...
describe('Matrix IDs', () => {
	...
	it('leaves the API port out, because a server name has none', () => {
		// /recover signs in with this string, so a port in it asks the
		// homeserver about a user that does not exist (#96).
		expect(matrixIdFor('michel', 'twalk.localhost:8009')).toBe('@michel:twalk.localhost');
