...
/**
 * The Matrix ID the deployment will create. Shown on screen 2, and — on
 * /recover, where the Gateway may not be able to name the owner — the id the
 * sign-in is actually made with.
 */
export function matrixIdFor(username: string, serverName: string): string {
	...
	// carry a port (`twalk.localhost:8009` on a tunnelled deployment). A Matrix
	// ID's domain is the **server name**, which is the host: building
	// `@michel:twalk.localhost:8009` produced a user the homeserver had never
	// heard of, and /recover reported it as a refused password (#96).
...
export function serverNameOf(domain: string): string {
	...
	return /^[0-9]+$/u.test(after) ? trimmed.slice(0, colon) : trimmed;
}
