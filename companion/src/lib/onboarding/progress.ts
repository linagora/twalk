...
// is remembered in `sessionStorage` beside the domain (`./domain.ts`), so that
...
export function rememberHomeserver(baseUrl: string): void {
	...
	try {
		window.sessionStorage.setItem(HOMESERVER_KEY, baseUrl);
	} catch {
...
export function restoreHomeserver(): string {
	try {
		const stored = window.sessionStorage.getItem(HOMESERVER_KEY);
