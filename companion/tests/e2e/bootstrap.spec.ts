...
test.describe.serial("the bootstrap journey", () => {
   ...
   */
  async function assertKeyNeverLeft(
    key: string,
    made: Request[] = requests,
  ): Promise<void> {
  ...
  test("screen 2 creates the account and shows the recovery key once", async () => {
    ...
    await expect(page.getByTestId("screen-recovery-key")).toBeVisible({
      timeout: 180_000,
    });
    ...
    await expect(page.getByTestId("key-backup-state")).toContainText(
      /key backup is on/i,
    );
  });
  ...
  async function createOwnerRoom(name: string): Promise<string> {
    ...
    const created = await fetch(
      `${stack!.synapseUrl}/_matrix/client/v3/createRoom`,
      {
        method: "POST",
        headers: {
  ...
  test("the journey carries on: a network, the assistant, the dashboard", async () => {
    ...
    await expect(page.getByTestId("card-matrix")).toHaveAttribute(
      "data-blocked",
      "",
    );
    ...
    await expect(page.getByTestId("screen-matrix")).toHaveAttribute(
      "data-stage",
      "rooms",
    );
    ...
    await expect(page.getByTestId("persona-assistant")).toHaveAttribute(
      "data-active",
      "yes",
    );
  ...
  test("a reload stays signed in", async () => {
    ...
    await expect(page.getByTestId("already-signed-in")).toContainText(
      stack.ownerId,
    );

    const stored = await page.evaluate(
      async (name) =>
        (await indexedDB.databases()).some((db) => db.name === name),
      CRYPTO_STORE_NAME,
    );
  ...
  async function expectDeviceIsCrossSigned(
    ...
    const query = await fetch(
      `${where.synapseUrl}/_matrix/client/v3/keys/query`,
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
    ...
    const keys = (await query.json()) as {
      device_keys?: Record<
        string,
        Record<string, { signatures?: Record<string, object> }>
      >;
      self_signing_keys?: Record<string, { keys: Record<string, string> }>;
    };

    const selfSigning = Object.values(
      keys.self_signing_keys?.[where.ownerId]?.keys ?? {},
    )[0];
    expect(
      selfSigning,
      "the account publishes a self-signing key",
    ).toBeDefined();
    ...
    const signed = Object.values(devices).filter((device) =>
      Object.keys(device.signatures?.[where.ownerId] ?? {}).includes(
        `ed25519:${selfSigning}`,
      ),
    );
    expect(
      signed.length,
      "a device is cross-signed on the homeserver",
    ).toBeGreaterThan(0);
  }
  ...
  test("a browser whose store was evicted is walked back in with the key", async () => {
    ...
    await expect(page.getByTestId("recover-account")).toContainText(
      stack.ownerId,
    );
    ...
    await expect(page.getByTestId("recover-done")).toBeVisible({
      timeout: 180_000,
    });
    ...
    await expect(page.getByTestId("recover-done")).toContainText(
      /open as before/i,
    );
  ...
  test("a browser that has never been here is asked which deployment", async ({
    browser,
  }) => {
    if (stack === null) {
      return;
    ...
    try {
      ...
      await expect(cold.getByTestId("recover-error")).toHaveAttribute(
        "data-kind",
        "unreachable",
      );
      ...
      await cold.getByTestId("recover-submit").click();
      await expect(cold.getByTestId("recover-done")).toBeVisible({
        timeout: 180_000,
      });
