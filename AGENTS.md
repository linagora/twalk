...
- `sensor/` — the Sensor Cargo package: library modules (`config`, `consent`, `network`, `normalize`, `outbound` — pure logic) plus the binary wiring to matrix-sdk and NATS in `src/main.rs`; integration-test harness in `tests/` (compose stack, bot/bus/contract helpers, `smoke.rs`, `sensor_lifecycle.rs`, `message_to_event.rs`, `outbound.rs`)
