...
- `companion/` — the Companion PWA: a SvelteKit + TypeScript static export (`adapter-static`, SPA fallback `200.html`, `trailingSlash: 'never'`), built by the Gateway image's Node stage and served from the Gateway's origin. The foundation (ticket #66): the design tokens (`src/lib/styles/`), the **generated** Gateway client (`src/lib/api/`, from `companion-gateway/openapi.yaml` — never hand-written, and `npm run api:check` fails when it is stale), the capability gate (`src/lib/capabilities/`), the version handshake against `/health` (`src/lib/version/`), French/English ICU catalogues (`src/lib/i18n/`), the Lucide indirection module (`src/lib/icons/`), the service worker, and screen 1 of the wireframes. Playwright is authoritative (`tests/e2e/`), over the built export served by `tests/serve-like-gateway.mjs` — which transcribes the Gateway's own path resolution. `companion/README.md` holds the decisions
