...
- `companion-gateway/` — the `twalk-companion-gateway` Cargo package: the service skeleton (ticket #48) — `config`, `http`, `static_files`, `metrics`, `trace` plus the binary in `src/main.rs`; the origin that serves the Companion's static files, `/health`, `/metrics` — and the user's session (ticket #52): `matrix_openid` (verifying a Matrix OpenID token at the homeserver, with the domain check), `session` (the owner check, the replay ledger, the device list and the per-device tokens in a SQLite store) and `session_http` (the session routes and the guard that requires a device token on every `/api` route by default) — and its own HTTP description (ticket #63): `companion-gateway/openapi.yaml`, an OpenAPI 3.1 description of the whole origin, served at `/openapi.yaml` by `openapi` (which embeds it) and **extended by every ticket that adds an endpoint**, because `tests/openapi.rs` fails on a route it does not describe. Integration tests in `companion-gateway/tests/` (`service.rs`, `signin.rs` and `openapi.rs` at the process boundary, `deployment.rs` against the compose service)
...
The Sensor and Hermes suites bring up the same test stack (one Synapse, one NATS JetStream), each on its own bus subjects; `TWALK_TEST_STACK`, `TWALK_TEST_SYNAPSE_PORT` and `TWALK_TEST_NATS_PORT` move a stack aside for a parallel worktree. The Companion Gateway's suite uses the same stack for its sign-in tests (real Synapse mints the OpenID tokens; the test Synapse serves the `openid` resource for it) and no bus yet; its process-boundary tests ask the kernel for a free port, and its deployment test uses the deploy-stack variables (`TWALK_DEPLOY_TEST_STACK`, `TWALK_DEPLOY_TEST_GATEWAY_PORT`) the Sensor's deployment test established. `sensor/tests/bridges_deployment.rs` runs the reference deployment a third time, with the `bridges` profile on, under its own `TWALK_BRIDGES_TEST_*` project, ports and teardown flag — its module docs list them.
