...
**A change to a shared path runs the consumers' suites.** `tests/harness/` is a dev dependency of three components and the Companion's real-stack e2e brings up its compose file; `contracts/cloudevents/v1/` is read by the harness's validator, the Sensor and the SDK's tests; `companion-gateway/openapi.yaml` is what the Companion's API client is generated from. That routing lives in `.github/ci/suites.json`, and `.github/ci/test_selection.py` derives each shared path's consumers from the repository rather than trusting the table — so if you add a dependency, the routing check will tell you to route it.
