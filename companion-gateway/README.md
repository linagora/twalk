...
| `GATEWAY_HERMES_ANSWER_SECRET` | *unset* | The secret **Hermes's** outbound hook signs each answer with (`POST /_twalk/hermes/answers`, ADR 0032, [#206](https://github.com/linagora/twalk/issues/206)). Unset: that route answers `503 hermes_answers_not_configured` and names the variable, because "Hermes does not answer" and "Hermes was never configured" are two sentences. Shorter than 32 characters: the Gateway refuses to start, on the service token's argument — it is the whole authentication of a route that creates suggestions about the user's conversations. |
| `GATEWAY_HERMES_DOMAIN` | *unset* | The authority of the `source` URI a suggestion published from an answer carries (`hermes://<domain>/personas/<id>`) — the persona runtime's own `HERMES_DOMAIN`. **Configuration and never a value taken from the push**: a signed answer is not a licence to name any source it likes. Required whenever the secret is set. |
| `GATEWAY_SUGGESTION_TTL_SECONDS` | `3600` | How long a suggestion published from one of Hermes's answers stays approvable. The same window the runtime gives the personas (`HERMES_SUGGESTION_TTL_SECONDS`) and the same default, because a suggestion that came through Hermes must not age differently from one a persona drafted itself ([#22](https://github.com/linagora/twalk/issues/22)). That the number now has two homes is a cost this ticket names rather than hides. |
