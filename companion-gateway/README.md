...
| anything else | The Companion's static files from `GATEWAY_STATIC_DIR`, and its `index.html` for any path that matches no file — the Companion is a static export with client-side routing, so a deep link reloaded cold loads the app. |
...
| `GATEWAY_STATIC_DIR` | *required* | Directory the Companion's static files are served from. Absent or empty is not fatal: the origin answers a clear `404` while health and metrics stay up. |
