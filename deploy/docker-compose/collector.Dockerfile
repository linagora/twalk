...
# the shared test harness (`tests/harness/`) as a path dependency — a dev
# dependency, but cargo needs it present to resolve the graph at all. The
# shared consent cache joins the COPY lines when lot 3's mail collector
# labels a sender with it.
...
RUN find . ../tests/harness ../contracts -type f \
