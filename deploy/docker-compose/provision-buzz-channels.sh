...
# Buzz a surface the owner holds. Hermes's *public* key is what
# `provision-hermes-nostr-key.sh` prints for the key it holds — that script is
# run here, and it is idempotent, so this one never derives anything and never
# touches the private half.
#
...
# The relay's **announced** URL (its RELAY_URL), and not whatever address also
# reaches it: a Buzz relay is multi-tenant by host, so a loopback URL and a
# public name are two communities with two rosters and two sets of channels,
# and NIP-98 binds every signed request to the announced URL besides. Hermes
# must use the same one (BUZZ_RELAY_URL in its env file).
...
grep -qE '^BUZZ_PRIVATE_KEY=' "$ENV_FILE" || { echo "no BUZZ_PRIVATE_KEY in $ENV_FILE: run provision-hermes-nostr-key.sh first" >&2; exit 1; }
# The sibling script prints the public half of the key the file holds, on a
# line reading `  hex   <64 hex>`; a second run writes nothing.
KEY_SCRIPT="$(cd "$(dirname "$0")" && pwd)/provision-hermes-nostr-key.sh"
HERMES_PUBKEY="$("$KEY_SCRIPT" "$ENV_FILE" 2>/dev/null | sed -nE 's/^ *hex +([0-9a-f]{64}).*/\1/p' | head -1)"
[ -n "$HERMES_PUBKEY" ] || { echo "could not read Hermes's public key from $KEY_SCRIPT" >&2; exit 1; }
