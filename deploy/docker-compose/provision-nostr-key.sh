#!/usr/bin/env bash
# Give a Buzz writer a Nostr signing key of its own.
#
#   ./provision-nostr-key.sh [env-file]   default: ~/.hermes-twalk/.env
#   ./provision-nostr-key.sh --self-test
#
...
# Not Hermes's: the clerk (#265) gets its key the same way, into its own file —
# `./provision-nostr-key.sh /etc/twalk/clerk.env`. The script takes an env file
# and writes `BUZZ_PRIVATE_KEY` into it, whoever reads that file; the old name
# `provision-hermes-nostr-key.sh` is kept as a symlink.
#
...
if grep -qE '^BUZZ_PRIVATE_KEY=' "$ENV_FILE"; then
	echo "$ENV_FILE already holds a Buzz key; leaving it untouched."
	...
	{
		printf '\n# The Nostr key this Buzz writer signs events with — its own identity, never the\n'
		printf '# owner'"'"'s (issue #239, ADR 0019, ADR 0032). Written by provision-nostr-key.sh.\n'
		printf 'BUZZ_PRIVATE_KEY=%s\n' "$(head -c 32 /dev/urandom | od -An -tx1 | tr -d ' \n')"
	} >> "$ENV_FILE"
	echo "Wrote a Buzz key into $ENV_FILE (mode 0600)."
...
The Buzz identity in $ENV_FILE (public half — the private half stays in the file):
  hex:  $pubkey
  npub: $npub
...
Then add this pubkey to the channels it should read, as the owner: Hermes is added
by ./provision-buzz-channels.sh from its env file, the clerk with --bot $pubkey.
EOF
