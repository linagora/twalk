...
1. **Do not convert anything. Copy `free`** (#379). Beside the busy intervals the answer carries the stretches nothing occupies: `start`/`end` in UTC, `start_local`/`end_local` in the owner's own zone, and `minutes`. Name a time **only** from the local pair. This is not a style preference: on 2026-09-26 an agent read the calendar three times, was told the zone, did the arithmetic itself, and proposed two hours that overlapped a meeting — it had found the gaps in the UTC intervals and written them as though they were local. An agent that copies cannot make that mistake. When `start_local` is absent, this deployment knows no zone: write your times in UTC and **say** they are UTC.
2. **Still name the zone**, and still count *next week* and *tomorrow* from `now` rather than from your own idea of today (#369): `timezone` is an IANA name, `timezone_source` says where it came from — `calendar` when the owner's calendar declares it, `events` when it is the zone most of their own events are written in, which is weaker and worth a little more caution in how you word an hour, and `now` is what time it is there right now, offset and all. Write every hour in that zone and name it — *"jeudi 12h30 (heure de Paris)"* — and work out *next week* and *tomorrow* from `now` rather than from your own idea of today: an agent that did that arithmetic itself once called 5–9 October "next week" when next week was 28 September. When those three are **absent**, this deployment knows no zone: write your times in UTC and say they are UTC. Never guess a zone — an hour in the wrong one reads perfectly and is wrong, and the owner approving it has nothing to notice.
3. **When the answer carries `working_day`, the gaps are already inside it** (#381): the owner has said which days they accept meetings on and how wide those days are, and `free` holds only what falls inside. So everything in `free` is offerable, and if `free` is empty on a week their calendar is not empty, it means their working hours that week are taken — say *that*, not "I have nothing at all". When `working_day` is absent they have said nothing, and judging which gap is worth offering is yours: a gap at 03:00 is free and is not an offer.
4. Propose **up to three** slots inside working hours that fall entirely in gaps, leaving a margin around busy intervals. Never propose a slot that overlaps one.
5. **Up to three means fewer when there are fewer.** A contact who asked for three has asked for three times, not for three sentences: if the week holds one decent slot, offer that one and say the week is full — *"je n'ai que jeudi 12h30 cette semaine-là, sinon la suivante"*. Measured on 2026-09-24: asked for three, an agent with one good gap filled the other two with 8:00 and 19:30 on a Friday. Both were genuinely free and neither is a time a person offers a colleague.
6. Say nothing about *why* the owner is busy — you do not know, and the answer does not carry it. "Thursday morning is taken" is right; "Thursday morning she has a meeting" is a guess.
7. If the window you need is wider than fourteen days, ask about the nearest fortnight first.
