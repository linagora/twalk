...
TOOLS = [
    {
        ...
        "description": (
            ...
            "**Use `free`, and copy it.** Beside the busy intervals the answer "
            "carries the stretches nothing occupies, each with `start`/`end` in "
            "UTC, `start_local`/`end_local` in the owner's own zone, and "
            "`minutes`. Name a time ONLY from `start_local`/`end_local`, never "
            "by converting an instant yourself: an agent that did that "
            "arithmetic proposed two hours that overlapped a meeting after "
            "reading the calendar correctly, because it wrote UTC gaps as if "
            "they were local hours. Prefer a gap of a decent `minutes`. When "
            "the answer carries `working_day`, the gaps are already inside the "
            "hours the owner accepts meetings in, so every one of them is "
            "offerable — and an empty `free` on a week whose calendar is not "
            "empty means their working hours are taken, which is what to say "
            "rather than \"I have nothing at all\". When it is absent they have "
            "said nothing and the judgement is yours: a gap at 03:00 is free "
            "and is not an offer. The answer also carries `timezone`, `timezone_source` and "
