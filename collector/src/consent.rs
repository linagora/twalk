...
//! Gateway's snapshot first, then the `consent.state.changed` stream from
...
//! with no Gateway has no snapshot and no decisions, and the cache stays
...
/// The cache, filled from the Gateway's snapshot document (the same one the
