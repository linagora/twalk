...
| `collector-stack` | required | **70 s**, 12 tests — 13.0 s `calendar`, 10.6 s `mail`, 25.5 s `push`, 11.1 s `reply`, 10.1 s `status` (measured 2026-09-20; ~180 s before #280 taught the tests to read from the bus head noted at their start) | — | — | warm stack; the process boundary against the bus, every poll at 1 s — except `push` (#277), which polls at 12 s to prove a delivery arrives inside the interval and waits one interval out with the socket cut |
