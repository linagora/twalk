...
| [ADR 0019](../architecture/adr/0019-a-persona-discloses-itself-to-the-contact.md) | Every reply a persona drafted discloses itself to the contact, in a sentence this project chooses, on by default and removable only on the record. Decided; no screen here offers it yet. |
...
**Shipped.** One component serves screens 3a and 3b (`$lib/components/QrLogin.svelte`), because the Gateway reports one shape for every bridge and the two screens differ only in their words. So screen 3a's correction applies here unchanged: polling, not Server-Sent Events; the QR drawn in the browser; `bridge.status.changed.v1` published by the Gateway from the bridge's own status push.
...
- The persona disclosure control ADR 0019 requires (decided, unbuilt).
