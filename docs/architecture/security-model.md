...
What the Companion Gateway deliberately does **not** hold, per [ADR 0011](adr/0011-gateway-authenticates-with-matrix-openid.md) — and, for the authentication half, now asserted rather than promised: `companion-gateway/tests/signin.rs` signs a device in against a real Synapse and then reads the Gateway's store and its captured log output for the tokens involved. No Matrix access token for the user, no network credentials (a bridge login's cookies or QR payload pass through and are forgotten), no recovery key, and no message content — its pending-contact projection stores a contact's Matrix ID, its network and timestamps, never a body and never a display name.
...
8. **One owner, no isolation between humans** (today, for authentication). A deployment serves one person on any number of devices: the Gateway accepts exactly the Matrix ID in `GATEWAY_OWNER` and refuses every other identity the homeserver would vouch for. There is no multi-user separation to lean on — every device of the owner has the same, total access to the Gateway's API — and the planned refusal to create a second account is also the weak point in any recovery story that loses the owner's identity. Note too what the owner check rests on: whoever controls the homeserver controls who the Gateway believes, because the homeserver is what answers the userinfo call.
