...
- **Designed, not built** — settled by an ADR or a spec issue, with no code behind it. The Companion PWA is in this state: `companion/` holds a README. The Companion Gateway is now **partly built**: its origin ([#48](https://github.com/linagora/twalk/issues/48)), its authentication — Matrix OpenID sign-in, the owner check and per-device tokens ([#52](https://github.com/linagora/twalk/issues/52)) — and its bootstrap — the registration relay for the first and only account, and the Sensor's invitation into the rooms the user chooses ([#53](https://github.com/linagora/twalk/issues/53)) — exist in `companion-gateway/`; consent and the bridge facade are still statements about what its spec commits it to hold ([#46](https://github.com/linagora/twalk/issues/46), [#47](https://github.com/linagora/twalk/issues/47)). The bridges have moved too: `mautrix-whatsapp` and `mautrix-signal` are part of the reference deployment as of [#73](https://github.com/linagora/twalk/issues/73), behind a compose profile, so the appservice credentials below are real files on a real volume — what is still designed-not-built is the Gateway's use of them.
...
| Each bridge's `as_token` (`WHATSAPP_AS_TOKEN`, `SIGNAL_AS_TOKEN`) | `.env`, the bridge's rendered config, the generated registration on the `bridge-registrations` volume (world-readable there, because Synapse reads it as its own user) — and, when it lands, the Gateway's configuration for verifying that bridge's status webhook | The bridge authenticates to the homeserver with it; the Gateway will check incoming webhooks against it | **Impersonating the appservice**: acting as any user in that bridge's namespace | Today (chosen by the operator, so that `.env` is the single source both halves come from) |
...
What the Companion Gateway deliberately does **not** hold, per [ADR 0011](adr/0011-gateway-authenticates-with-matrix-openid.md) — and, for the authentication and bootstrap halves, now asserted rather than promised: `companion-gateway/tests/signin.rs` signs a device in against a real Synapse, and `companion-gateway/tests/bootstrap.rs` creates the owner's account and invites the Sensor into a room, and both then read the Gateway's store and its captured log output for every credential involved — the user's Matrix access token, the OpenID token, the password the user chose and the registration shared secret itself. No Matrix access token for the user, no network credentials (a bridge login's cookies or QR payload pass through and are forgotten), no recovery key, and no message content — its pending-contact projection stores a contact's Matrix ID, its network and timestamps, never a body and never a display name.
...
2. **The Companion Gateway concentrates the homeserver's most powerful credentials** (the registration secret today when the relay is enabled; the `as_token` designed, not built). It can create an account and it will hold a credential that impersonates an appservice. Compromising it reaches further than compromising the Companion, the bus, or any single bridge. Its own mitigations — refusing accounts after the first, using the `as_token` only to verify webhooks — constrain what the Gateway does, not what its secrets would allow.
...
5. **Message metadata accumulates in the Gateway's contact projection** (designed, not built). No bodies and no display names, but who contacted the user, on which network, and when — a social graph with timestamps, in a service reachable from the browser. Note also that a bridged Matrix ID conventionally embeds the network identifier (`@whatsapp_33612345678:example.com`), so storing the Matrix ID keeps the phone number even though the projection stores no identifier field of its own.
...
7. **`pending` is enforced by consumers, not by the Sensor** (today). A persona that ignores the label reads bodies it was never granted.
...
9. **One owner, no isolation between humans** (today, for authentication). A deployment serves one person on any number of devices: the Gateway accepts exactly the Matrix ID in `GATEWAY_OWNER` and refuses every other identity the homeserver would vouch for. There is no multi-user separation to lean on — every device of the owner has the same, total access to the Gateway's API — and the refusal to create a second account ([#53](https://github.com/linagora/twalk/issues/53)) is also the weak point in any recovery story that loses the owner's identity. Note too what the owner check rests on: whoever controls the homeserver controls who the Gateway believes, because the homeserver is what answers the userinfo call.
10. **A remote LLM moves message content outside the infrastructure** (designed, not built). Consent gates whether a persona reads an event; it does not gate where that persona reasons. An operator who configures a hosted model has consented to that, and should know they did.
