...
- **`revoked`.** The Sensor still publishes an inbound event, but without the message body: the event keeps its identity, network, timestamps and consent label, and collects nothing ([ADR 0012](adr/0012-revoked-consent-reduces-publication.md)). Publishing nothing at all was rejected: the user would lose the evidence that a message arrived, and a broken bridge would become indistinguishable from a silent contact. The contract change that makes a body-less inbound event legal is [#58](https://github.com/linagora/twalk/issues/58), still open.

**What `revoked` stops:** future publication of that contact's message bodies, from the moment the Gateway records the decision and the Sensor applies it.
