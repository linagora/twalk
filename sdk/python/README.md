...
| `TWALK_LLM_PARAMS` | `{}` | A JSON object of provider parameters, merged into every request untouched — because providers differ in what they reject. A parameter set to `null` *removes* a field the request would otherwise carry (`{"temperature": null}`), which is how an endpoint that refuses one of them is made to work. **Every** request, including the language ask (below): merged last, `{"max_tokens": N}` replaces the ask's own five-token budget as well as the reply's. **A reasoning model needs it set** — one that thinks before it answers spends the ask's five tokens thinking, every time, and the persona then refuses every suggestion after drafting each reply, naming the ask and this variable. |
...
**What crosses is a narrow template of named fields** — ten of them, listed in `webhook.py` — and never the raw event. Hermes keeps memories and nothing expires what it is shown, so a body copied into `MEMORY.md` outlives ADR 0028's seven days on a machine Twalk may not own. The template names the message's words and the shape of the conversation around them, and withholds everybody's identity: the sender's Matrix ID, their display name, the network's own identifier for them, the portal room, the quoted excerpt that belongs to whoever wrote it, and an attachment's name or decryption material. A persona author cannot widen it, for the same reason they cannot forget the consent gate. `tests/test_webhook.py` asserts the whole key set and then searches the serialised body for each withheld value.
