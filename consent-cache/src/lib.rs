//! The consent cache: the pure half of deciding consent, shared by every
//! component that labels a sender (ADR 0033, issue #273) — the Sensor today,
//! the mail collector next — so that two components cannot disagree about
//! what a decision means.
//!
//! Consent is the data-processing agreement state of a contact, or of a whole
//! connection (CONTEXT.md). The Companion Gateway is the single writer of
//! consent state (ADR 0006); a consumer only labels events with the current
//! state, held here in an in-memory cache keyed on `(subject, connection)`.
//!
...
//!    sequence it reflects — read once at startup;
...
//!
//! What is **not** here is the I/O: reading the snapshot over HTTP and
//! consuming the stream are each component's own, because a Sensor and a
//! collector reach the bus and the Gateway differently. What is here is
//! every decision — the parsing of an entry and of a change, the refusal of
//! what names no connection, the exclusion of the owner's identities and the
//! bridges' bots ([`owner`], [`bridge_bot`]), the precedence between a
//! contact's own decision and the connection's default — so that a test can
//! prove two components agree by running one crate.
...
/// The Gateway's consent snapshot as the cache reads it: the whole current
/// state, what in it could not be read, and the stream sequence a consumer
/// applying it carries on from.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Snapshot {
    pub entries: Vec<ConsentEntry>,
    /// The entries the Gateway served that this cache could not label a
    /// sender by, and why — `NoConnection` is the one an operator must hear
    /// about: a Gateway older than #270 serves a state this cache will not
...
mod tests {
    ...
    fn a_snapshot_carries_contacts_defaults_and_the_position_to_follow_from() {
        let snapshot = Snapshot::parse(&snapshot_document(
            41,
            json!([
                {
    ...
    fn an_empty_snapshot_starts_the_consumer_at_the_beginning_of_the_stream() {
        let snapshot = Snapshot::parse(&snapshot_document(0, json!([]))).unwrap();
        assert_eq!(snapshot.entries, Snapshot::empty().entries);
    ...
    fn a_snapshot_without_a_position_is_refused() {
        ...
        let error = Snapshot::parse(&document).unwrap_err().to_string();
        assert!(error.contains("next_stream_sequence"), "{error}");
        // And so is an answer that is not a snapshot at all.
        assert!(Snapshot::parse(&json!({ "error": "unauthenticated" })).is_err());
    }
}
