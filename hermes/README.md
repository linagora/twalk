...
- `tests/harness/` — the Hermes-specific half of the harness: `PersonaRun` (the persona alone, no runtime) and `RuntimeRun` (the runtime, starting the persona itself).

Still to come: the approval API ([#24](https://github.com/linagora/twalk/issues/24)), which is where a suggestion's expiry is enforced and where a deliberate second attempt would be asked for.
