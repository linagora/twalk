...
Making that address reachable is **not** the runtime's job: how a persona's container joins a network is the command's business and the deployment's. It belongs in `deploy/docker-compose/`, which does not run Hermes yet.
