...
Still to come: the runtime that starts and supervises persona processes ([#23](https://github.com/linagora/twalk/issues/23)) — which is also what injects the LLM configuration and the user's language preference the Gateway holds (ADR 0015, ADR 0016) — and the approval API ([#24](https://github.com/linagora/twalk/issues/24)), which is where a suggestion's expiry is enforced and where a deliberate second attempt would be asked for.
